...
export const IS_MOBILE = /iPhone|iPad|iPod|Android|BlackBerry|BB10/i.test(
  navigator.userAgent,
)
...
export function coordinatesToPoint(
  ...
  return new Vector3(
    -globeRadius * Math.sin(a) * Math.cos(b),
    globeRadius * Math.cos(a),
    globeRadius * Math.sin(a) * Math.sin(b),
  )
}

export function coordinatesToRotation(
  latitude: number,
  longitude: number,
): Quaternion {
...
export function getImageData(texture: HTMLImageElement): ImageData {
  ...
  if (ctx === null) throw new Error('Unable to get a 2D context')
